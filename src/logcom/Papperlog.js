...
function App() {
  ...
  const [shol, setShol] = useState(true);
...
function Lowest(a,b){
  ...
  console.log(location.state[0].price)
          var profud = (location.state.map((item,hanel)=>item))
          setOlpk(profud.sort((a, b) => a.price - b.price))
      console.log(olpk);
...
function Higest(){
  ...
  setSeller(false)
  var profud = (location.state.map((item,hanel)=>item))
  setJklp(profud.sort((a, b) => b.price - a.price))
